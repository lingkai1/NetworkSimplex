...
void NetworkMaxFlowSimplex::solve() {
	...
	while (true) {
		...
		while (u != source) {
			...
			NodeID p = ar.head;
#if defined(LEAVING_ARC_NEAR_S) || defined(LEAVING_ARC_FARFROM_PIVOT)
			if (ai.resCap <= delta) {
#elif defined(LEAVING_ARC_NEAR_T) || defined(LEAVING_ARC_NEAR_PIVOT)
				if (ai.resCap < delta) {
#endif
					delta = ai.resCap;
					xy = i;
			...
			while (u != sink) {
				...
				NodeID p = ai.head;
#if defined(LEAVING_ARC_NEAR_T) || defined(LEAVING_ARC_FARFROM_PIVOT)
				if (ai.resCap <= delta) {
#elif defined(LEAVING_ARC_NEAR_S) || defined(LEAVING_ARC_NEAR_PIVOT)
					if (ai.resCap < delta) {
#endif
						delta = ai.resCap;
						xy = i;
		...
		void NetworkMaxFlowSimplex::relabelPhase() {
#if !defined(RELABEL_OPT)
			while (qr.dmin < qt.dmin) {
#else
			while (qr.dmin-1 < qt.dmin) {
#endif
				NodeID v = qr.extractMin();
				if (v == UNDEF_NODE) break;
				relabel(v);
			}
		}
	...
	return false;*/
		...
		bool NetworkMaxFlowSimplex::makeCur(NodeID v) {
			...
			while (nv.cur < nodes[v+1].first) {
				...
				ArcID uv = avu.rev; Arc& auv = arcs[uv];
				if (isPseudoResidual(nu, nv, uv, vu, auv) && nv.d == nu.d + 1) {
